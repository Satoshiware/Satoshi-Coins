...
// todo: the truncated search still seams to be case sensitive. Spacing is fine.
//todo: Update only works on non-frozen chains from the database. ---------- this came from the test file
require "admin.php";
...
if(!empty($_GET["search"])) {
    ...
    } else if (strtolower(substr($_GET["search"], 0, 4)) == "cmd:") { // Was it an administrator's command?
        // Trim leading and trailing spaces of the parameters and then make sure there is only a one space in between.
        adminCMD(preg_replace('/\s+/', ' ', trim(substr($_GET["search"], 4))));
    }
}
?>
